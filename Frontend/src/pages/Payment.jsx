...
//             total: product.price * (item.count || item.quantity || 1),
//             phone: address.phone,
//             address,
//             method,
...
function Payment() {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      for (let item of cart) {
        const product = item.productData || item.productId;
        totalAmount += (product?.price || 0) * (item.count || 1);
      }
      ...
      if (method === "cod" || method === "upi") {
        for (let item of cart) {
          ...
          const paymentData = {
            ...
            total: (product?.price || 0) * (item.count || item.quantity || 1),
            phone: address.phone,
            address,
            method,
            upiId: method === "upi" ? upiId : "",
